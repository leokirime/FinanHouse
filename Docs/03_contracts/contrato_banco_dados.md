...
| financial_entries ⚠️ | installment_plan_id | BIGINT UNSIGNED | Não | FK composta (installment_plan_id, household_id) → installment_plans(id, household_id), RESTRICT; nulo/preenchido sempre junto com installment_number (CHECK, DT-17) |
| financial_entries ⚠️ | installment_number | BIGINT UNSIGNED | Não | posição da parcela (1..installment_count do plano); único por plano via índice composto (installment_plan_id, installment_number) (DT-17) |
| installment_plans ⚠️ | id | BIGINT UNSIGNED AUTO_INCREMENT | Sim | PK |
| installment_plans ⚠️ | household_id | BIGINT UNSIGNED | Sim | FK → households.id, RESTRICT |
| installment_plans ⚠️ | description | VARCHAR(255) | Sim | |
| installment_plans ⚠️ | category_id | BIGINT UNSIGNED | Sim | FK composta (category_id, household_id) → categories(id, household_id), RESTRICT |
| installment_plans ⚠️ | total_amount | DECIMAL(13,2) | Sim | positivo, CHECK |
| installment_plans ⚠️ | installment_count | BIGINT UNSIGNED | Sim | >= 2, CHECK |
| installment_plans ⚠️ | first_reference_month | DATE | Sim | dia 1 do mês da primeira parcela; sem FK para monthly_periods (competência pode não existir ainda, DT-17) |
| installment_plans ⚠️ | due_day | BIGINT UNSIGNED | Sim | entre 1 e 31, CHECK; nunca opcional (correção do Bloco 02) |
| installment_plans ⚠️ | created_by_user_id | BIGINT UNSIGNED | Sim | FK → users.id, RESTRICT; só autoria/auditoria, nunca filtro de visibilidade |
| installment_plans ⚠️ | — | — | — | unique(id, household_id) como alvo da FK composta de financial_entries; imutável (sem update()/remove() na porta) — DT-17 |
...
Todas as tabelas têm `created_at`/`updated_at` (TIMESTAMP), exceto `auth_sessions` (só `created_at`, sem `updated_at` — `last_used_at`/`revoked_at` cobrem as mutações relevantes) e `installment_plans` (só `created_at` — plano imutável, nunca `UPDATE`d). Detalhes completos, índices e regras: `database/proposed-schema/`. `category_budgets` aplicada a `finanhouse_dev` em 2026-08-04 (Bloco 18, DT-13); `users.password_hash`/`password_configured_at` e `auth_sessions` aplicadas em 2026-08-06 (Bloco 19, DT-14/DT-15). ⚠️ = `installment_plans` e as colunas `installment_plan_id`/`installment_number` de `financial_entries` definidos em `apps/api/src/db/schema/` e migration `0004_deep_machine_man.sql` gerada/revisada, mas **ainda não aplicados** a `finanhouse_dev` — não assumir sua presença até a aplicação ser confirmada (ver seção 8).
...
- `0004_deep_machine_man.sql` (Sessão 12, Bloco 03, DT-17) foi gerada e revisada (`drizzle-kit generate`/`check`: "Everything's fine") em 2026-08-20 — **não aplicada** a nenhum ambiente; nenhum `drizzle-kit migrate` foi executado neste bloco. Cria a tabela `installment_plans` e adiciona `installment_plan_id`/`installment_number` (nullable) a `financial_entries`. Aplicação depende de autorização explícita do proprietário em bloco futuro.
...
- ~~`installment_plans` (parcelamentos)~~ — **modelado e migration gerada em 2026-08-20** (Sessão 12, Bloco 03, DT-17): tabela `installment_plans` + extensão nullable de `financial_entries` (`installment_plan_id`/`installment_number`); aplicação a `finanhouse_dev` pendente de autorização explícita do proprietário. Persistência atômica de plano + N parcelas (RS-01) continua pendente — ver Sessão 12, Bloco 04.
