...
Todas as tabelas têm `created_at`/`updated_at` (TIMESTAMP), exceto `auth_sessions` (só `created_at`, sem `updated_at` — `last_used_at`/`revoked_at` cobrem as mutações relevantes) e `installment_plans` (só `created_at` — plano imutável, nunca `UPDATE`d). Detalhes completos, índices e regras: `database/proposed-schema/`. `category_budgets` aplicada a `finanhouse_dev` em 2026-08-04 (Bloco 18, DT-13). ⚠️ = `users.password_hash`/`password_configured_at`, `auth_sessions`, `installment_plans` e as colunas `installment_plan_id`/`installment_number` de `financial_entries` definidos em `apps/api/src/db/schema/` e migrations `0003_auth_sessions.sql`/`0004_deep_machine_man.sql` geradas/revisadas, mas **ainda não aplicados** a `finanhouse_dev` — não assumir sua presença até a aplicação ser confirmada (ver seção 8).
