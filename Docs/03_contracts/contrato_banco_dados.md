...
| users | password_hash | VARCHAR(255) | Não | hash Argon2id; nulo até a senha ser configurada (DT-14, migration `0003` aplicada em 2026-08-06) |
| users | password_configured_at | TIMESTAMP | Não | preenchido junto com `password_hash` |
...
| financial_entries | installment_plan_id | BIGINT UNSIGNED | Não | FK composta (installment_plan_id, household_id) → installment_plans(id, household_id), RESTRICT; nulo/preenchido sempre junto com installment_number (CHECK, DT-17) |
| financial_entries | installment_number | BIGINT UNSIGNED | Não | posição da parcela (1..installment_count do plano); único por plano via índice composto (installment_plan_id, installment_number) (DT-17) |
| installment_plans | id | BIGINT UNSIGNED AUTO_INCREMENT | Sim | PK |
| installment_plans | household_id | BIGINT UNSIGNED | Sim | FK → households.id, RESTRICT |
| installment_plans | description | VARCHAR(255) | Sim | |
| installment_plans | category_id | BIGINT UNSIGNED | Sim | FK composta (category_id, household_id) → categories(id, household_id), RESTRICT |
| installment_plans | total_amount | DECIMAL(13,2) | Sim | positivo, CHECK |
| installment_plans | installment_count | BIGINT UNSIGNED | Sim | >= 2, CHECK |
| installment_plans | first_reference_month | DATE | Sim | dia 1 do mês da primeira parcela; sem FK para monthly_periods (competência pode não existir ainda, DT-17) |
| installment_plans | due_day | BIGINT UNSIGNED | Sim | entre 1 e 31, CHECK; nunca opcional (correção do Bloco 02) |
| installment_plans | created_by_user_id | BIGINT UNSIGNED | Sim | FK → users.id, RESTRICT; só autoria/auditoria, nunca filtro de visibilidade |
| installment_plans | — | — | — | unique(id, household_id) como alvo da FK composta de financial_entries; imutável (sem update()/remove() na porta) — DT-17 |
...
| auth_sessions | id | BIGINT UNSIGNED AUTO_INCREMENT | Sim | PK |
| auth_sessions | user_id | BIGINT UNSIGNED | Sim | FK → users.id, RESTRICT |
| auth_sessions | household_id | BIGINT UNSIGNED | Sim | FK → households.id, RESTRICT |
| auth_sessions | token_hash | VARCHAR(64) | Sim | SHA-256 hex do token de sessão, único — nunca o token bruto (DT-14) |
| auth_sessions | expires_at | TIMESTAMP | Sim | 7 dias após a criação |
| auth_sessions | revoked_at | TIMESTAMP | Não | preenchido no logout |
| auth_sessions | last_used_at | TIMESTAMP | Não | atualizado a cada validação de sessão bem-sucedida |
| auth_sessions | — | — | — | índices em `user_id`, `expires_at`, único em `token_hash`; vínculo do usuário com o household validado na aplicação, não por FK (DT-14) |

Todas as tabelas têm `created_at`/`updated_at` (TIMESTAMP), exceto `auth_sessions` (só `created_at`, sem `updated_at` — `last_used_at`/`revoked_at` cobrem as mutações relevantes) e `installment_plans` (só `created_at` — plano imutável, nunca `UPDATE`d). Detalhes completos, índices e regras: `database/proposed-schema/`. `category_budgets` aplicada a `finanhouse_dev` em 2026-08-04 (Bloco 18, DT-13); `users.password_hash`/`password_configured_at` e `auth_sessions` aplicadas em 2026-08-06 (Bloco 19, DT-14/DT-15); `installment_plans` e as colunas `installment_plan_id`/`installment_number` de `financial_entries` aplicadas em 2026-08-20 (Sessão 12, Bloco 03, DT-17/DT-18, migration `0004_deep_machine_man.sql`). Nenhuma marcação ⚠️ de "ainda não aplicado" permanece nesta tabela.
...
- `0003_auth_sessions.sql` (Bloco 19, DT-14) foi gerada e revisada (`drizzle-kit check`: "Everything's fine") em 2026-08-04 e **aplicada ao banco real de desenvolvimento (`finanhouse_dev`) em 2026-08-06**, com duas autorizações explícitas e separadas do proprietário (checkpoint do Bloco 19) — `__drizzle_migrations` agora com quatro migrations registradas. Só adicionou colunas/tabela (`users.password_hash`/`password_configured_at`, `auth_sessions`); a configuração das senhas em si foi uma segunda autorização separada, via `db-configure-initial-passwords.ts`. Produção continua sem nenhuma migration aplicada.
- `0004_deep_machine_man.sql` (Sessão 12, Bloco 03, DT-17) foi gerada e revisada (`drizzle-kit generate`/`check`: "Everything's fine") em 2026-08-20 e **aplicada ao banco real de desenvolvimento (`finanhouse_dev`) na mesma data**, com autorização explícita do proprietário — `__drizzle_migrations` agora com cinco migrations registradas. Cria a tabela `installment_plans` e adiciona `installment_plan_id`/`installment_number` (nullable) a `financial_entries`; auditoria somente leitura antes/depois confirmou as oito tabelas estruturais anteriores com contagens preservadas e `installment_plans` criada vazia. Produção continua sem nenhuma migration aplicada.
...
- ~~`users.password_hash`/`password_configured_at` + `auth_sessions` (autenticação real)~~ — **encerrada em 2026-08-06** (Bloco 19, DT-14/DT-15): migration `0003_auth_sessions.sql` aplicada a `finanhouse_dev` com duas autorizações explícitas e separadas do proprietário; senhas iniciais dos dois usuários existentes configuradas via `db-configure-initial-passwords.ts`.
- ~~`installment_plans` (parcelamentos)~~ — **encerrada em 2026-08-20** (Sessão 12, Bloco 03, DT-17/DT-18): tabela `installment_plans` + extensão nullable de `financial_entries` (`installment_plan_id`/`installment_number`) modeladas, migration gerada e, com autorização explícita do proprietário, aplicada a `finanhouse_dev` na mesma data. Persistência atômica de plano + N parcelas (RS-01) continua pendente — ver Sessão 12, Bloco 04.
- Extensões futuras não modeladas ainda: `recurrence_rules`, `period_status_history` — ver `database/proposed-schema/extensoes-futuras.md`.
