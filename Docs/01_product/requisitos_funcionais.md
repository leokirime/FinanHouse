...
| RF-05 | Persistir movimentações e competências em banco real (MySQL) | Must | Persistência real em andamento — infraestrutura Aiven e schema disponíveis: TLS validado em 2026-07-30, migration inicial aplicada em 2026-07-31 a `finanhouse_dev` (seis tabelas, Bloco 12, DT-08), integridade composta do membro responsável corrigida em 2026-07-31 (FK+CHECK, Bloco 13, DT-09), e repositórios Drizzle reais implementados e validados por smoke-test transacional em 2026-07-31 (Bloco 14, DT-10) — infraestrutura, schema e camada de persistência concluídos. **Ainda não concluído**: endpoints de API HTTP e integração do frontend com a API continuam pendentes — a aplicação segue lendo/escrevendo apenas o estado em memória |
