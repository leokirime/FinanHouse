...
| RF-05 | Persistir movimentações e competências em banco real (MySQL) | Must | Persistência real em andamento — infraestrutura Aiven e schema disponíveis: TLS validado em 2026-07-30 e migration inicial aplicada em 2026-07-31 a `finanhouse_dev` (seis tabelas, Bloco 12, DT-08), ambas as pendências de infraestrutura encerradas. **Ainda não concluído**: repositórios Drizzle reais, endpoints de API e integração do frontend com a API continuam pendentes — a aplicação segue lendo/escrevendo apenas o estado em memória |
