...
| RF-05 | Persistir movimentações e competências em banco real (MySQL) | Must | Persistência real em andamento — infraestrutura Aiven e schema disponíveis: TLS validado em 2026-07-30, migration inicial aplicada em 2026-07-31 a `finanhouse_dev` (seis tabelas, Bloco 12, DT-08), e integridade composta do membro responsável corrigida em 2026-07-31 (FK+CHECK, Bloco 13, DT-09) — todas as pendências de infraestrutura/schema encerradas. **Ainda não concluído**: repositórios Drizzle reais, endpoints de API e integração do frontend com a API continuam pendentes — a aplicação segue lendo/escrevendo apenas o estado em memória |
