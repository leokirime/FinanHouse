...
| `bloco_11_migracao_da_infraestrutura_mysql_para_aiven` | Migração da infraestrutura MySQL para Aiven | Concluído (preparação de código/arquitetura) — branch `feat/session-11-bloco-11-aiven-database`, push realizado, **sem merge à `main`** (instrução explícita); sem conexão real, migration ou seed executados |
...
- **TLS/SSL não verificado com o provedor real (P2)** — diagnosticado como incompatível na Clever Cloud (Bloco 02 usou `DATABASE_SSL=false` apenas para ler metadados) e nunca corrigido ali; desde o Bloco 11 o alvo passa a ser o Aiven, com configuração/TLS/CA já preparados (DT-07), mas a pendência só fecha quando um `db:check` real, com CA, confirmar TLS ativo. Continua em aberto. Registrado em ADR-001 e `Docs/03_contracts/contrato_banco_dados.md`.
