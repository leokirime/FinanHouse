...
**Refinamento visual do hero da competência em 2026-07-31 (`bloco_15_refinamento_visual_do_hero_da_competencia`):** correção pontual de UX solicitada entre o Bloco 14 e o próximo bloco funcional. O hero da competência mensal (`HeroBrand.tsx`) usava um grid de duas colunas em que a logo, envolvida por um painel `.fh-hero__brand-surface` (fundo quase-branco, borda, sombra), ocupava ~40% da seção. Corrigido: removidos o grid e o painel; a logo passou a ser um elemento decorativo `position: absolute` no canto superior direito (150–210px no desktop, reduzindo por breakpoint até 90–120px no mobile), e a competência (título, descrição, status, ação) passou a ser o único conteúdo em fluxo normal. Durante o diagnóstico, uma suposição inicial de que a imagem não tinha transparência real foi verificada e corrigida por inspeção binária direta do PNG (decodificação de IHDR + scanlines): o arquivo é RGBA com alpha real — o problema era inteiramente estrutural (o painel), não do asset. Tokens de design órfãos (`--fh-brand-surface`/`--fh-brand-surface-border`) removidos. Nenhuma regra funcional, estado, rota, API ou dado foi alterado. Branch `fix/session-11-hero-competencia-logo`, integrada à `main` em 2026-07-31 (merge `17fb9ba`, a partir do commit `9ab913e`, branch preservada no remoto). Não iniciado o próximo bloco funcional (endpoints HTTP).
