...
export interface CategoryBudgetListProps {
  ...
  onRemoveLimit: (row: CategoryBudgetRowViewModel) => void
}

/**
 * Distribuição de despesas previstas por categoria, com limite mensal real
 * (Bloco 18, DT-13) — `row.limit`/`row.status`/`row.percentConsumed` vêm de
 * `buildPlanningViewModel` alimentado com os limites reais da API (nunca um
 * `CategoryBudget` em memória).
 */
export function CategoryBudgetList({ rows, canManageLimits, pendingAction, onDefineLimit, onEditLimit, onRemoveLimit }: CategoryBudgetListProps) {
  if (rows.length === 0) {
    return (
                ...
                <td data-label="Ações">
                  <div className="fh-planning-list__actions">
                    {row.hasLimit && row.budgetId !== null ? (
                      <>
                        <button type="button" onClick={() => onEditLimit(row)} disabled={!canManageLimits || pendingAction}>
                          Editar limite
                        </button>
                        <button type="button" onClick={() => onRemoveLimit(row)} disabled={!canManageLimits || pendingAction}>
                          Remover limite
                        </button>
                      </>
                    ) : (
                      <button
                        type="button"
                        onClick={() => onDefineLimit(row)}
                        disabled={!canManageLimits || pendingAction}
                        title={canManageLimits ? undefined : 'Limites só podem ser gerenciados em competências abertas ou em revisão.'}
                      >
                        Definir limite
                      </button>
                    )}
