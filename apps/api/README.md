...
## API HTTP (Bloco 16, DT-11; limites por categoria no Bloco 18, DT-13)

Fastify 5.11.0 sobre os repositórios Drizzle reais (`src/http/`) — contrato completo em `Docs/03_contracts/contrato_api_http.md`. Desde o Bloco 18, inclui `.../periods/:referenceMonth/budgets` (GET/PUT/DELETE) para limites mensais por categoria — implementado e testado, mas depende da migration `0002_category_budgets.sql` já aplicada a `finanhouse_dev` (ver seção "Banco de dados" abaixo). Pontos de segurança obrigatórios:
